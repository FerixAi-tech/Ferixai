...
export const ADMIN_PANEL_COOKIE = "ferixai_admin_panel";

function getSessionSecret(): string {
  return (
    process.env.FERIXAI_ADMIN_PANEL_SECRET?.trim() ||
    process.env.SUPABASE_SERVICE_ROLE_KEY?.trim() ||
    "ferixai-admin-panel-dev"
  );
}

export function getAdminPanelPassword(): string | null {
  const password = process.env.FERIXAI_ADMIN_PANEL_PASSWORD?.trim();
  return password || null;
}

export function createAdminPanelSessionToken(): string {
  return createHmac("sha256", getSessionSecret())
    .update("ferixai-admin-panel-session")
    .digest("hex");
}
